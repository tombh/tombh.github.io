...
A [viewshed](https://en.wikipedia.org/wiki/Viewshed) is just all the things that you can see from any given point. Here is an example from my home country, Wales, looking across a large estuary towards England:
...
I think it's good enough for our needs, but I'm curious as to what an _optimal_ packing would look like. Let me outline my approach and maybe you can come up with something better.
...
The only notable optimisation I've added is for step 3. Instead of just automatically adding any overlapping tile, I instead first see if extending an _existing_ nearby TVS tile to cover the uncovered subtile would introduce less new surface area than the new overlapping tile itself. But this introduces an interesting problem: arbitrarily extending an existing tile might make it cover a new elevation that triggers a greater theoretical maximum line of sight, which in turn requires the tile to be extended yet again to ensure all its viewsheds are valid. This is fine for my not-terrible approach, but it adds complexity to the search space when a requirement of the algorithm is to actually optimise for the lowest possible total area of overlaps across the world.

I'm sure all this could just be brute forced to find ever improved optimums. There are only a few thousand tiles after all. Though currently my implementation takes about 2 hours to tile the world, which doesn't make brute forcing so approachable. I'm sure you could do some localised gradient descent and then apply the found input variable optimums globally. But perhaps there's an elegant efficient solution? This isn't the first packing problem to exist, I imagine there are similar problems that could inform this one. If you're interested in exploring then there are only 3 rules that an algorithm needs to follow:

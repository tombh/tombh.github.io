...
_Some tips: I know that most OSs have a shortcut for a 'runner', eg; ALT+F2 on most Linuxes gives you a text input where you can type `xterm` or similar, on OSX it's ALT+SPACE to get a runner. You can assume you get focus on a command prompt after starting your terminal app. As far as I know, an SSH daemon is already installed on OSX, you just need to start it, if anyone can provide the steps for that I'll include them here._
